...
const Navbar = ({pages}) => {//need to pass the hyperlink (address to page) as well in props
  ...
  return (
    <div>
        ...
        >
        <div
          className='px-8 border rounded-lg flex flex-row justify-center items-center w-[auto] h-[44px] space-x-4'
          style={{ 
        ...
        >
                {pages.map((sectionTitle,index) => (
                  <div>
                    <p><a href={`${frontend}/admin/products`}>{sectionTitle}</a></p>
                  </div>
                ))}
            </div>
        </div>
